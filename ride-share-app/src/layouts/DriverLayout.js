...
const DriverLayout = () => {
    ...
    const navItems = [
        ...
        { name: 'Post a Ride', path: '/driver/post-ride', icon: PlusCircle },
        { name: 'Vehicle Management', path: '/driver/vehicles', icon: Car },
        { name: 'Upcoming Rides', path: '/driver/rides', icon: CalendarClock },
                ...
                <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
                    {navItems.map((item) => (
                        <NavLink
                            key={item.path}
                            to={item.path}
                            end={item.path === '/driver'} // Only match exact path for dashboard root
                            onClick={() => setIsSidebarOpen(false)}
                            className={({ isActive }) =>
